...
/*static void printGrammer(map<string, Definition>& grammar) {
    map<string, Definition>::iterator it = grammar.begin();
...
static void printDefinition(map<string, Definition>& grammar, string def) {
    Production prod = grammar[def].getRandomProduction();
    vector<string>::iterator it = prod.begin();
    while (it!=prod.end()) {
        string st = *it;
        if (st[0]=='<') {
            printDefinition(grammar, st);
        } else {
            cout << st << " ";
        }
        it++;
